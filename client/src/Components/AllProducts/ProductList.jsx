...
const ProductList = () => {
    ...
    const fetchProducts = async () => {
        try {
            const { data } = await axios.get('http://localhost:3000/products', { params: filters })
            setProducts(data)
        } catch {
    ...
    const handleFilterChange = (newFilters) => setFilters(prevFilters => ({ ...prevFilters, ...newFilters }))
    ...
    const handleAddToCart = async (productId) => {
        ...
        try {
            ...
            Swal.fire({ icon: 'error', title: 'Error', text: 'An error occurred while adding to the cart' })
        }
    }

    const handleRarityChange = (e) => handleFilterChange({ rarity: e.target.value })
                        ...
                        <div key={product.id} className="product-card">
                            ...
                            <div className="product-info">
                                <div className="product-header">
                                    <span className={`rarity-badge ${product.rarity.toLowerCase().replace(' ', '-')}`}>{product.rarity}</span>
